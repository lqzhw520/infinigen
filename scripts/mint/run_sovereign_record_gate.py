...
def run() -> bool:
    ...
    ev_content = (
        ...
        f"  seeds across all 15 seeds × 6 branches. All legal rollouts achieved\n"
    ...
    if r.returncode != 0:
        print(f"[FAIL] record-evidence E012: {r.stderr[:300]}")
    else:
        print(f"[OK] record-evidence E012")
    ...
    r = run_cli(
        ...
        "stats. Both drawer_robot_env.py and drawer_robot_env_mujoco.py updated."
    )
    records.append({
    ...
    r = run_cli(
        ...
        "--status", "supported",
        "--change-reason",
        "P1b: DrawerRobotEnvMujoco provides LIBERO-aligned dual-camera observations "
        "(256x256 agentview + wrist). P1b gate PASS."
    )
    records.append({
    ...
    r = run_cli(
        ...
        f"13 physics-legal rollouts saved. ready_for_v58_training.",
